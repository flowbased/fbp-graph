...
class Journal extends EventEmitter {
  constructor(graph, metadata, store) {
    ...
    this.graph.on('addNode', (node) => this.appendCommand('addNode', node));
    this.graph.on('removeNode', (node) => this.appendCommand('removeNode', node));
    ...
    this.graph.on('changeNode', (node, oldMeta) => this.appendCommand('changeNode', { id: node.id, new: node.metadata, old: oldMeta }));
    this.graph.on('addEdge', (edge) => this.appendCommand('addEdge', edge));
    this.graph.on('removeEdge', (edge) => this.appendCommand('removeEdge', edge));
    this.graph.on('changeEdge', (edge, oldMeta) => this.appendCommand('changeEdge', {
      from: edge.from, to: edge.to, new: edge.metadata, old: oldMeta,
    }));
    this.graph.on('addInitial', (iip) => this.appendCommand('addInitial', iip));
    this.graph.on('removeInitial', (iip) => this.appendCommand('removeInitial', iip));
    ...
    this.graph.on('startTransaction', (id, meta) => this.startTransaction(id, meta));
    this.graph.on('endTransaction', (id, meta) => this.endTransaction(id, meta));
  }
  ...
  startTransaction(id, meta) {
    ...
    this.currentRevision += 1;
    this.appendCommand('startTransaction', { id, metadata: meta }, this.currentRevision);
  }
  ...
  moveToRevision(revId) {
    ...
    if (revId > this.currentRevision) {
      ...
      let asc; let end; let
        start;
      for (start = this.currentRevision + 1, r = start, end = revId, asc = start <= end;
        asc ? r <= end : r >= end;
        asc ? r += 1 : r -= 1) {
      ...
      let end1;
      for (r = this.currentRevision, end1 = revId + 1; r >= end1; r -= 1) {
        this.store.fetchTransaction(r).forEach((entry) => {
